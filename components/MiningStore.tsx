...
type RarityId = "common" | "rare" | "epic" | "legendary";
type StoreTab = "cards" | "boosts";
type SortBy = "cost" | "income" | "rarity";
...
const resolveRarity = (card: StoreCard) => {
  const raw = String(card.rarity || card.rarity_level || card.tier || "")
    .trim()
    .toLowerCase();
...
interface MiningStoreProps {
  ...
  userCards?: OwnedCard[];
  balanceKi?: number;
  onPurchaseCard?: (card: StoreCard) => boolean;
  loadingText?: string;
  boosts?: StoreBoost[];
  activeBoost?: ActiveBoost;
  onPurchaseBoost?: (boost: StoreBoost) => void;
}
...
export default function MiningStore({
  ...
  const [storeTab, setStoreTab] = useState<StoreTab>("cards");
  ...
  const cardRows = useMemo<DisplayCard[]>(() => {
    const rows = cards.map((card) => {
      ...
      const cost =
        level > 0 ? Math.floor(baseCost * Math.pow(1.5, level)) : baseCost;
      const rarityDef = resolveRarity(card);
      return {
        ...card,
        owned,
        level,
    ...
    return rows.sort((a, b) => {
      ...
      if (sortBy === "cost") {
        ...
        valA = a.incomePerSecond;
        valB = b.incomePerSecond;
      } else if (sortBy === "rarity") {
        valA = a.rarityDef.threshold;
        valB = b.rarityDef.threshold;
      }
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-5">
          <AnimatePresence>
            {cardRows.map((card) => {
              ...
              const nextIncomePerSecond =
                card.incomePerSecond * Math.max(1, card.level || 1);
                ...
                >
                  ...
                  <div className="relative z-10 flex h-full flex-col">
                    <div className="absolute right-3 top-3 z-20">
                      <span
                        className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.22em] ${card.rarityDef.badge} bg-slate-950/70 backdrop-blur-md`}
                      >
                    ...
                    <div className="relative flex-1">
                      <div
                        className={`absolute inset-0 bg-gradient-to-b ${card.rarityDef.glow}`}
                      />
                      <div
                        className={`absolute inset-0 bg-slate-950/20 ${!isOwned ? "backdrop-blur-[1px] brightness-[0.45] saturate-0" : ""}`}
                      />
                    ...
                    <div className="relative z-10 -mt-12 rounded-[24px] border border-white/10 bg-slate-950/90 p-4 shadow-[0_-12px_30px_rgba(0,0,0,0.35)] backdrop-blur-xl">
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p
                            className={`mt-0.5 text-sm font-black ${card.rarityDef.accent}`}
                          >
                            +{formatKi(nextIncomePerSecond)}/s
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-slate-500">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-medium text-slate-400">
                          {isOwned
                            ? `Next +${formatKi(card.incomePerSecond)} /s`
                            : "Base income shown"}
                        </span>
                      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {boostRows.map((boost) => {
            ...
            return (
              <article
                key={boost.id}
                className={`group relative overflow-hidden rounded-[28px] border bg-gradient-to-b from-orange-500/15 to-slate-950/90 shadow-[0_20px_60px_rgba(0,0,0,0.35)] ${isLocked ? "border-white/5" : "border-orange-500/25"}`}
              >
